...
if [ ${UCP_CNT} == 0 ]; then
    ...
    # Set up UCP HA
    echo "******************** Creating UCP cluster"
    docker-machine create --driver vmwarefusion --vmwarefusion-memory-size 4096 ucp0 && docker-machine ssh ucp0 docker swarm init --advertise-addr $(docker-machine ip ucp0)
    ...
    do
        ...
        docker-machine create --driver vmwarefusion --vmwarefusion-memory-size 4096 ucp$COUNT && docker-machine ssh ucp$COUNT docker swarm join --token $(docker-machine ssh ucp0 docker swarm join-token -q manager) $(docker-machine ip ucp0)
    done
    echo "----------- Installing UCP ------------------"
...
if [ $3 == 0 ]; then
    ...
    do
        ...
        docker-machine create --driver vmwarefusion --engine-label environment=$ENVWORKERLABEL node$COUNT  && echo "Created node$COUNT" && echo node$COUNT >> swarmnodes && docker-machine ssh node$COUNT docker swarm join --token $(docker-machine ssh ucp0 docker swarm join-token -q worker) $(docker-machine ip ucp0)
    done
fi
...
if [ ${DTR_CNT} == 0 ]; then
    ...
    docker-machine create --driver vmwarefusion --vmwarefusion-memory-size 3072 dtr0
    # Join Swarm
    ...
    do
        ...
        docker-machine create --driver vmwarefusion --vmwarefusion-memory-size 3072 dtr$COUNT && docker-machine ssh dtr$COUNT docker swarm join --token $(docker-machine ssh ucp0 docker swarm join-token -q worker) $(docker-machine ip ucp0)
